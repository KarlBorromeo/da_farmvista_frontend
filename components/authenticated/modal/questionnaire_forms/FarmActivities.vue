		...
		<v-container>
			<form-card>
				...
				<v-row>
                    ...
                    <form-radio-container title="Did know proper reutilization of agri-waste?">
                    ...
                    <form-checkbox-container v-if="didKnowProperReutilization == 'no'" title="If No, where diposed the unutilzied agir-waste?">
                        <v-checkbox
                    ...
                    <form-checkbox-container title="Where heared about reutilization of agri-waste">
                        <v-checkbox
...
export default {
	...
	methods: {
		...
		validateRadio() {
			if (this.agriculturalSystem.length == 0 ||
				this.didKnowProperReutilization.length == 0 ||
				this.whereHearAboutReutilization.length == 0){
				return false
			}
            if(this.didKnowProperReutilization == 'no' && this.whereDisposedUnutilizedAgriwaste.length == 0){
                return false
            }
			return true
		},
		...
		getData() {
			return {
                agriculturalSystem: this.concatOtherValue(this.agriculturalSystem,this.agriculturalSystemOther),
                knowProperReutilizationAgriwaste: this.didKnowProperReutilization,
                whereDisposedUnutilizedAgriwaste: this.concatOtherValue(this.whereDisposedUnutilizedAgriwaste,this.whereDisposedUnutilizedAgriwasteOther),
                whereHearAboutReutilization: this.concatOtherValue(this.whereHearAboutReutilization,this.whereHearAboutReutilizationOther),
			}
		},
	},
	beforeMount() {
		this.agriculturalSystemItems = this.$store.getters['questionnaireCode/AgriculturalSystem'];
        this.whereDisposedUnutilizedAgriwasteItems = this.$store.getters['questionnaireCode/WasteDisposalMethod'];
        this.whereHearAboutReutilizationItems = this.$store.getters['questionnaireCode/AgriWasteReutilizationInformationSource'];
	},
}
</script>
